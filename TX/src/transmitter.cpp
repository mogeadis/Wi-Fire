...
void initializeTX()
{
    ...
    lcd_msg("SYSTEM BOOTING",2,1,0);
    delay(MSG_DELAY/2);
    if(radio.begin() && radio.isChipConnected())
    {
        Serial.println("SUCCESSFUL\n");
        lcd_msg("SUCCESSFUL",4,2);
        delay(MSG_DELAY);
    }
    else
    {
        Serial.println("UNSUCCESSFUL\n");
        lcd_msg("UNSUCCESSFUL",3,2);
        delay(MSG_DELAY);
        while (true){}
    }
...
void request_status(byte node,byte channel)
{
    packet.id++;
    packet.node = node;
...
void arm_channels(byte node,byte channel)
{
    packet.id++;
    packet.node = node;
    packet.channel = channel;
...
void fire_channels(byte node,byte channel)
{
    packet.id++;
    packet.node = node;
    packet.channel = channel;
