...
void loop()
{
    ...
    if(key)
    {
        ...
        switch(key)
        {
            ...
            {
                num_of_selected = 1;
                selection = new byte[num_of_selected]{char_to_byte(key)};
                break;
            }
            case '7':
            {
            ...
            case '0':
            {
                digitalWrite(RED_LED,HIGH);
                digitalWrite(GREEN_LED,HIGH);
                lcd_msg("SYSTEM",6,1,0);
                lcd_msg("LOCKED",6,2,-1);
