...
// Define Functions
void lcd_msg(const char msg[],byte col,byte row,byte clear)
{
...
void lcd_custom(byte character,byte col,byte row)
{
    lcd.setCursor(col-1,row-1);
    lcd.write(character);
}

byte char_to_byte(char CHAR)
{
    byte BYTE = CHAR - '0';
    return BYTE;
}
